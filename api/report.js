...
// 뉴스: 서버에서 RSS XML 직접 파싱 (rss2json 제거)
// Claude 2회 호출: ①시장분석 ②뉴스요약
...
export default async function handler(req, res) {
  ...
  // ── 6. 뉴스 RSS 직접 파싱 (서버사이드 XML 파싱) ─────────
  // RSS XML에서 title/link/pubDate 추출
  function parseRSS(xml, type, source) {
    ...
    for (const item of itemMatches.slice(0, 8)) {
      ...
      if (title && title.length > 5) {
        items.push({ title, link: link || '', pubDate: dateMatch?.[1]?.trim() || '', type, source });
      }
    }
    return items;
  }
  ...
  await Promise.allSettled(rssFeeds.map(async ({ url, type, source }) => {
    try {
      ...
      const items = parseRSS(xml, type, source);
      rawNews.push(...items);
    } catch(e) {}
  }));
  ...
  const uniqueNews = rawNews.filter(n => {
    const k = n.title.slice(0, 20).toLowerCase();
...
순수 JSON만 출력 (마크다운 없이):
{
  ...
  "calendar":[
    ...
    {"date":"2026-03-29","event":"이벤트명","country":"US","importance":"low","description":"설명"},
    {"date":"2026-03-30","event":"이벤트명","country":"KR","importance":"medium","description":"설명"}
  ],
  "earnings":[
    {"company":"기업명","ticker":"티커","market":"US또는KR","date":"날짜","epsEstimate":"예상EPS","result":"예정또는발표","signal":"upcoming또는beat또는miss또는meet","importance":"high또는medium"},
...
calendar는 오늘(${today})부터 7일간 실제 주요 경제지표 일정, importance 순 정렬.
earnings는 이번 주 국내+해외 주요 기업 실적 10개 이상, importance 순 정렬.
brokerageReports는 최근 주요 증권사 목표가 변경 5개, importance 순 정렬.`,
        }],
      }),
      signal: to(50000),
    });
    ...
    if (r1.ok) {
      ...
      if (d.error) {
        ...
        if (s > -1 && e > s) {
          ...
          claudeError = 'JSON 없음 — 응답: ' + raw.slice(0, 100);
        }
      }
    } else {
    ...
    try {
      const r2 = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `아래 뉴스 헤드라인 ${live.RAW_NEWS.length}개를 분석하여 투자자 관점에서 요약하세요.

${newsLines}

순수 JSON 배열만 출력 (마크다운 없이):
[
  {"index":0,"summary":"투자자 관점 2문장 한국어 요약","category":"매크로또는금리또는환율또는반도체또는에너지또는암호화폐또는기업실적또는정치또는기타","importance":"high또는medium또는low"}
]

모든 ${live.RAW_NEWS.length}개를 빠짐없이. importance는 시장 영향도 기준.`,
          }],
        }),
        signal: to(50000),
      });
      ...
      if (r2.ok) {
        ...
        if (!d2.error) {
          ...
          if (s2 > -1 && e2 > s2) {
            try { newsSummaries = JSON.parse(clean2.slice(s2, e2+1)); }
            catch(e) {}
          }
        }
      }
  ...
  if (!analysis) {
    analysis = {
      headline: '시장 데이터 수집 완료',
      sentiment: '중립', sentimentReason: claudeError || 'AI 분석 실패',
  ...
  return res.status(200).json({
    ...
    headline: analysis.headline, sentiment: analysis.sentiment,
    sentimentReason: analysis.sentimentReason, riskLevel: analysis.riskLevel,
    riskReason: analysis.riskReason, strategy: analysis.strategy,
    ...
    crypto: {
      ...
      fundingRate: live.BTC_FUNDING, fearGreed: live.FEAR_GREED,
    ...
    volatility: {
      ...
      riskLevel: analysis.riskLevel, analysis: analysis.volatilityAnalysis,
    },
    calendar: sortedCal, earnings: sortedEarn, brokerageReports: sortedBrok,
    news: enrichedNews,
    insights: {
